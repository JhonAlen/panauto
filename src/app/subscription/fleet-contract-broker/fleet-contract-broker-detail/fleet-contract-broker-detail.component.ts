...
async getmetodologia(){
  ...
  verifyPayment() {
    if (this.cordenUbii > 0) {
      ...
      window.alert('Primero Debe de pagar para poder generar el reporte del Contrato.')
    }
  }
  ...
  async getFleetContractDetail(ccontratoflota) {
    ...
    await this.http.post(`${environment.apiUrl}/api/fleet-contract-management/detail`, params, options).subscribe( async (response: any) => {
      if(response.data.status){
        ...
        await window.alert(`Se ha generado exitosamente el Contrato n° ${this.xpoliza} del cliente ${this.xnombrecliente} para el vehículo de placa ${this.xplaca}. Presione Aceptar para generar el Contrato en formato PDF`);
        this.clear = true;
        try {this.createPDF()}
