...
@Component({
  selector: 'app-club-contract-management-index',
  templateUrl: './club-contract-management-index.component.html',
  styleUrls: ['./club-contract-management-index.component.css']
})
export class ClubContractManagementIndexComponent implements OnInit {
  ...
  onSubmit(form){
    ...
    this.http.post(`${environment.apiUrl}/api/v2/club-contract-management/production/search`, params, options).subscribe((response : any) => {
      if(response.data.status){
        this.clubContractList = [];
        for(let i = 0; i < response.data.list.length; i++){
          this.clubContractList.push({ 
  ...
  goToDetail(){
    this.router.navigate([`subscription/club-contract-management-detail`]);
  }

  rowClicked(event: any){
    this.router.navigate([`subscription/club-contract-management-detail/${event.data.ccontratoclub}`]);
  }
