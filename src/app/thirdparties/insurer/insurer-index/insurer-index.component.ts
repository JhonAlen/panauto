...
@Component({
  selector: 'app-insurer-index',
  templateUrl: './insurer-index.component.html',
  styleUrls: ['./insurer-index.component.css']
})
export class InsurerIndexComponent implements OnInit {
  ...
  onSubmit(form){
    ...
    let params = {
      ...
      xaseguradora: form.xaseguradora ? form.xaseguradora : undefined,
      xdocidentidad: form.xdocidentidad ? form.xdocidentidad : undefined
    }
    this.http.post(`${environment.apiUrl}/api/insurer/search`, params, options).subscribe((response : any) => {
      if(response.data.status){
        this.insurerList = [];
        for(let i = 0; i < response.data.list.length; i++){
          this.insurerList.push({ 
            caseguradora: response.data.list[i].caseguradora,
            xaseguradora: response.data.list[i].xaseguradora,
  ...
  goToDetail(){
    this.router.navigate([`thirdparties/insurer-detail`]);
  }

  rowClicked(event: any){
    this.router.navigate([`thirdparties/insurer-detail/${event.data.caseguradora}`]);
  }
