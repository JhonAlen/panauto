...
@Component({
  selector: 'app-proficient-index',
  templateUrl: './proficient-index.component.html',
  styleUrls: ['./proficient-index.component.css']
})
export class ProficientIndexComponent implements OnInit {
  ...
  onSubmit(form){
    ...
    let params = {
      ...
      xperito: form.xperito ? form.xperito : undefined,
      xrazonsocial: form.xrazonsocial ? form.xrazonsocial : undefined,
      xdocidentidad: form.xdocidentidad ? form.xdocidentidad : undefined
    }
    this.http.post(`${environment.apiUrl}/api/proficient/search`, params, options).subscribe((response : any) => {
      if(response.data.status){
        this.proficientList = [];
        for(let i = 0; i < response.data.list.length; i++){
          this.proficientList.push({ 
            cperito: response.data.list[i].cperito,
            xperito: response.data.list[i].xperito,
  ...
  goToDetail(){
    this.router.navigate([`thirdparties/proficient-detail`]);
  }

  rowClicked(event: any){
    this.router.navigate([`thirdparties/proficient-detail/${event.data.cperito}`]);
  }
