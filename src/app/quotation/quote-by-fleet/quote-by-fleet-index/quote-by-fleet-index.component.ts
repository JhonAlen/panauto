...
@Component({
  selector: 'app-quote-by-fleet-index',
  templateUrl: './quote-by-fleet-index.component.html',
  styleUrls: ['./quote-by-fleet-index.component.css']
})
export class QuoteByFleetIndexComponent implements OnInit {
  ...
  onSubmit(form){
    ...
    this.http.post(`${environment.apiUrl}/api/quote-by-fleet/search`, params, options).subscribe((response : any) => {
      if(response.data.status){
        this.quoteByFleetList = [];
        for(let i = 0; i < response.data.list.length; i++){
          this.quoteByFleetList.push({ 
            ccotizadorflota: response.data.list[i].ccotizadorflota,
  ...
  goToDetail(){
    this.router.navigate([`quotation/quote-by-fleet-detail`]);
  }

  rowClicked(event: any){
    this.router.navigate([`quotation/quote-by-fleet-detail/${event.data.ccotizadorflota}`]);
  }
