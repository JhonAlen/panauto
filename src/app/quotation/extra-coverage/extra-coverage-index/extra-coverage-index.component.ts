...
@Component({
  selector: 'app-extra-coverage-index',
  templateUrl: './extra-coverage-index.component.html',
  styleUrls: ['./extra-coverage-index.component.css']
})
export class ExtraCoverageIndexComponent implements OnInit {
  ...
  onSubmit(form){
    ...
    this.http.post(`${environment.apiUrl}/api/extra-coverage/search`, params, options).subscribe((response : any) => {
      if(response.data.status){
  ...
  goToDetail(){
    this.router.navigate([`quotation/extra-coverage-detail`]);
  }

  rowClicked(event: any){
    this.router.navigate([`quotation/extra-coverage-detail/${event.data.ccoberturaextra}`]);
  }
