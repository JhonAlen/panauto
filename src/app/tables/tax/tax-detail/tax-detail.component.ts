...
export class TaxDetailComponent implements OnInit {
  ...
  async getTaxData(): Promise<void> {
    ...
    if (request.data.status) {
      this.detail_form.get('ximpuesto').setValue(request.data.ximpuesto);
  ...
  editTax() {
    ...
    this.detail_form.get('ximpuesto').enable();
  ...
  cancelSave() {
    if (this.code) {
      ...
      this.router.navigate([`/tables/tax-index`]);
    }
  }
  async onSubmit(form): Promise<void> {
    ...
    if (this.code) {
      params = {
        ...
        cimpuesto: this.code,
        ximpuesto: form.ximpuesto,
        xobservacion: form.xobservacion,
        bactivo: form.bactivo,
        cusuariomodificacion: this.currentUser.data.cusuario,
        cpais: this.currentUser.data.cpais
      };
      //  url = `${environment.apiUrl}/api/tax/update`;
      request = await this.webService.updateTax(params);
    } else {
      params = {
        permissionData: {
          cusuario: this.currentUser.data.cusuario,
      ...
      // url = `${environment.apiUrl}/api/tax/create`;
      request = await this.webService.createTax(params);
    }
    if (request.error) {
      this.alert.message = request.message;
      this.alert.type = 'danger';
    ...
    if (request.data.status) {
      if (this.code) {
        ...
        this.router.navigate([`/tables/tax-detail/${request.data.cimpuesto}`]);
      }
    } else {
      let condition = request.data.condition;
      if (condition == "tax-name-already-exist") {
        this.alert.message = "TABLES.TAXES.NAMEALREADYEXIST";
        this.alert.type = 'danger';
        this.alert.show = true;
      }
    }
